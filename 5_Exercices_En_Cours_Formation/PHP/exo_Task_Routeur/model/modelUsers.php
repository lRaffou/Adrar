...
function readUsersAll($bdd){
    try{
        ...
        $req=$bdd->prepare('SELECT id_users, name_user, first_name_user, login_user FROM users');
...
function readUserByLogin($bdd,$login){
    try{
        ...
        $req=$bdd->prepare('SELECT id_users, name_user, first_name_user, login_user, mdp_user FROM users WHERE login_user = ?');
